function intensity_profile_GUI
    ...
    function drawProfileLine(~, ~)
        ...
        % Hitung θ (theta) berdasarkan gradien garis yang ditarik
        ...
        % Tampilkan θ pada plot
        text(0.8*max(dist), 0.8*max(c), ['θ = ', num2str(theta, '%.2f'), '°'], ...
    ...
    %% Fungsi untuk Menghitung θ (theta)
    function theta = calculateTheta(x, y, cx, cy)
        % Hitung gradien pada titik awal dan akhir dari garis yang digambar
        dx = cx(2) - cx(1);
        dy = cy(2) - cy(1);
        
        % Hitung α (alpha) dan θ (theta)
        alpha = atan2(dy, dx) * (180 / pi);  % Sudut dalam derajat
        theta = mod(alpha + 90, 360); % Sudut normal (90° ditambahkan untuk tegak lurus)
    end
