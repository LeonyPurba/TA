function ellipse_improve()
    ...
    function autobutton_Callback(hObject, ~)
       ...
       % rms_diff = sqrt(mean((r2 - r1).^2));
       % --- Tambahan analisis deviasi r2 - r1
       %deviation = r2 - r1;
       
       % Hitung nilai statistik
       %mean_deviation = mean(deviation)
       %std_deviation = std(deviation);
       %rms_deviation = sqrt(mean(deviation .^ 2));

       % Tampilkan ke Command Window
       %fprintf('Mean deviation     : %.4f\n', mean_deviation);
       %fprintf('Standard deviation : %.4f\n', std_deviation);
       %fprintf('RMS deviation      : %.4f\n', rms_deviation);
       
       % (Opsional) Tampilkan juga di GUI
       %msg = sprintf('Mean: %.4f | STD: %.4f | RMS: %.4f', ...
           %mean_deviation, std_deviation, rms_deviation);
       %title(['r1 & r2 on \theta — ' msg]);  % Menambahkan info ke judul plot
